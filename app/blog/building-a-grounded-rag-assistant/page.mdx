...
export const post = getPostBySlug("building-a-grounded-rag-assistant");
...
<header className="mt-8 mb-10 border-b border-border pb-8">
  ...
  <div className="mt-6 flex flex-wrap gap-2">
    {/* Lead action — same outlined structure as the others so the button
        row reads as a cohesive group, but the primary-colored icon +
        left-aligned border accent mark it as the main CTA. Outlined
        buttons work reliably in both light and dark mode without any
        text-on-primary contrast problems. */}
    <a
...
I spent a year at HCLTech as a Tier 1 Google Maps Platform support engineer. Every day, the same questions came in: *RefererNotAllowedMapError*, *OVER_QUERY_LIMIT*, *how do I migrate off `google.maps.Marker`*, *does the $200 credit still exist*. Most of the answers were in Google's docs — the developers couldn't find them, or got tripped up by outdated Stack Overflow posts that referenced APIs that had been renamed or deprecated.

So I built a RAG chatbot to answer those questions. The promise of RAG — *Retrieval-Augmented Generation* — is that you avoid hallucination by grounding the model's answers in documentation you control. Sounds simple. Took me several major iterations and a fundamental architecture rethink to actually deliver on it.
